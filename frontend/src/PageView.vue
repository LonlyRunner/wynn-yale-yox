...
function openCatChat(){chatOpen.value=true;chatError.value='';if(!chatMessages.value.length)chatMessages.value.push({role:'assistant',content:text('你好呀，我是团子 🐾 可以陪你聊天，也能看图片、读取文档和听语音。','Hi, I’m Tuanzi 🐾 I can chat, inspect images, read documents, and listen to audio.')});loadChatModels();loadChatHistory();scrollChat()}
...
<main :class="['site',{dark}]">
  ...
  <section v-if="chatOpen" class="cat-chat" role="dialog" aria-modal="false" :aria-label="text('团子聊天助手','Tuanzi chat assistant')">
    ...
    <div class="cat-chat-toolbar"><label>{{text('对话模型','Model')}}<select v-model="chatModel"><option v-for="model in chatModels" :key="model.id" :value="model.id">{{model.name}}</option></select></label><label v-if="chatVoices.length" class="chat-voice-select">{{text('可爱女声','Cute voice')}}<select v-model="chatVoiceName" @change="selectChatVoice"><option v-for="voice in chatVoices" :key="`${voice.name}-${voice.lang}`" :value="voice.name">{{voice.name}} · {{voice.lang}}</option></select></label><button type="button" :class="{active:chatAutoSpeak}" @click="toggleAutoSpeak">🔊 {{text('自动朗读','Auto voice')}}</button><button v-if="chatOwner" type="button" @click="clearChatHistory">{{text('清空记录','Clear history')}}</button></div>
    <div class="cat-chat-messages"><article v-for="(message,index) in chatMessages" :key="index" :class="message.role"><span>{{message.role==='assistant'?'🐾':'你'}}</span><div><div class="chat-content" v-html="markdown(message.content)"></div><button v-if="message.role==='assistant'" class="chat-speak" type="button" @click="speakChat(message.content)">🔈 {{text('朗读','Read')}}</button><small v-if="message.sources?.length">{{text('参考：','Sources: ')}}{{message.sources.map(source=>source.title).join(' · ')}}</small></div></article><article v-if="chatBusy" class="assistant thinking"><span>🐾</span><div><p>{{text('团子正在理解附件并翻知识库…','Tuanzi is reading attachments and searching the knowledge base…')}}</p></div></article></div>
