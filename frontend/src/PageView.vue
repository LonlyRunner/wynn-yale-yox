...
const chatOwner=computed(()=>sessionStorage.getItem('wynn-auth')==='1')
...
async function loadChatHistory(){if(chatHistoryLoaded.value||!chatOwner.value)return;chatHistoryLoaded.value=true;try{const items=await api<ChatMessage[]>('/chat/history');if(items.length)chatMessages.value=items}catch{}finally{scrollChat()}}
...
function speakChat(value:string){if(!('speechSynthesis' in window))return;window.speechSynthesis.cancel();const speech=new SpeechSynthesisUtterance(value.replace(/[#*_`>\[\]()]/g,' '));speech.lang=lang.value==='zh'?'zh-CN':'en-US';speech.rate=.98;window.speechSynthesis.speak(speech)}
...
onMounted(()=>{addEventListener('keydown',key);addEventListener('resize',resizeAdminCharts);addEventListener('resize',updateCompactMusicPlayer);updateCompactMusicPlayer();loadPublic();if(route.name==='admin')loadAdmin();if(route.name==='game'&&gameId.value==='pelican')loadChatModels()});onBeforeUnmount(()=>{removeEventListener('keydown',key);removeEventListener('resize',resizeAdminCharts);removeEventListener('resize',updateCompactMusicPlayer);disposeAdminCharts();clearInterval(timer.value);clearInterval(aiPollTimer);window.speechSynthesis?.cancel()})
...
<main :class="['site',{dark}]">
  ...
  <section v-if="chatOpen" class="cat-chat" role="dialog" aria-modal="false" :aria-label="text('团子聊天助手','Tuanzi chat assistant')">
    ...
    <div class="cat-chat-toolbar"><label>{{text('对话模型','Model')}}<select v-model="chatModel"><option v-for="model in chatModels" :key="model.id" :value="model.id">{{model.name}}</option></select></label><button type="button" :class="{active:chatAutoSpeak}" @click="toggleAutoSpeak">🔊 {{text('自动朗读','Auto voice')}}</button><button v-if="chatOwner" type="button" @click="clearChatHistory">{{text('清空记录','Clear history')}}</button></div>
