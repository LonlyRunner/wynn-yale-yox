...
<main :class="['site',{dark}]">
  ...
  <section v-else-if="route.name==='share'" class="share-page">
    ...
    <div class="share-grid">
      <article class="share-card"><div class="share-card-meta"><span aria-hidden="true">✳</span><small>01 / MODEL API</small></div><h2>{{text('火锅中转站','Huoguo API relay')}}</h2><p>{{text('模型中转站的注册入口。','Registration link for a model API relay.')}}</p><a href="https://huoguo.cc.cd/register?aff=MK3KP5KD7DXJ" target="_blank" rel="noopener noreferrer sponsored">{{text('打开火锅中转站','Open Huoguo')}} ↗</a></article>
      <article class="share-card"><div class="share-card-meta"><span aria-hidden="true">⌁</span><small>02 / NETWORK</small></div><h2>{{text('魔戒','Mojie')}}</h2><p>{{text('网络工具的两个注册入口，任选其一。','Two registration links for the same network service. Choose either one.')}}</p><div class="share-actions"><a href="https://mojie.co/register?aff=dtwKRNVc" target="_blank" rel="noopener noreferrer sponsored">{{text('入口一','Link one')}} ↗</a><a href="https://mojie.co/register?aff=jr9Cy7pz" target="_blank" rel="noopener noreferrer sponsored">{{text('入口二','Link two')}} ↗</a></div></article>
    </div>
...
.share-card a:hover { background: #925848; transform: translateY(-2px); }
