...
function uploadToOss(url:string,file:File,item:UploadItem){return new Promise<void>((resolve,reject)=>{const xhr=new XMLHttpRequest();xhr.open('PUT',url);xhr.setRequestHeader('Content-Type',file.type||'application/octet-stream');xhr.upload.onprogress=progress=>{if(progress.lengthComputable)item.progress=Math.min(99,Math.round(progress.loaded/progress.total*100))};xhr.onload=()=>xhr.status>=200&&xhr.status<300?resolve():reject(new Error(`HTTP ${xhr.status}`));xhr.onerror=()=>reject(new Error(text('网络连接失败','Network error')));xhr.send(file)})}
async function uploadGallery(event:Event){const input=event.target as HTMLInputElement,files=Array.from(input.files||[]);if(!files.length)return;uploads.value=files.map((file,index)=>({id:`${Date.now()}-${index}`,name:file.name,progress:0,status:'uploading'}));adminNotice.value=text('正在上传图片…','Uploading images…');for(const [index,file] of files.entries()){const item=uploads.value[index];try{const key=`gallery/${Date.now()}-${index}-${file.name.replace(/[^a-zA-Z0-9._-]/g,'-')}`;const signed=await api<{url:string}>('/admin/oss/upload-url',{method:'POST',body:JSON.stringify({objectKey:key,contentType:file.type})});await uploadToOss(signed.url,file,item);item.progress=100;item.status='processing';await api('/admin/media',{method:'POST',body:JSON.stringify({objectKey:key,titleZh:file.name.replace(/\.[^.]+$/,''),titleEn:file.name.replace(/\.[^.]+$/,''),mediaType:'IMAGE',sortOrder:galleryItems.value.length+index})});item.status='done'}catch(error){item.status='failed';item.error=aiError(error)}}const failed=uploads.value.filter(item=>item.status==='failed').length;adminNotice.value=failed?text(`${failed} 张上传失败，请重试`,`${failed} upload(s) failed; try again`):text('上传完成，AI 提示词已生成','Upload complete; AI prompts generated');input.value='';await loadPublic()}
...
<main :class="['site',{dark}]">
  ...
  <div class="admin-workspace"><form class="post-editor" @submit.prevent="saveKnowledge"><div class="editor-head"><h2>{{knowledgeForm.id?text('编辑知识条目','Edit entry'):text('新增知识或人格','New knowledge or persona')}}</h2><button type="button" @click="resetKnowledge">＋ {{text('清空','Reset')}}</button></div><div class="form-grid"><label>{{text('类型','Type')}}<select v-model="knowledgeForm.kind"><option value="KNOWLEDGE">{{text('知识库','Knowledge')}}</option><option value="PERSONA">{{text('人格','Persona')}}</option></select></label><label>{{text('状态','Status')}}<select v-model="knowledgeForm.enabled"><option :value="true">{{text('启用','Enabled')}}</option><option :value="false">{{text('停用','Disabled')}}</option></select></label><label class="wide">{{text('标题','Title')}}<input v-model="knowledgeForm.title" required></label><label class="wide">{{text('标签（逗号分隔）','Tags')}}<input v-model="knowledgeForm.tags"></label><label class="wide">{{knowledgeForm.kind==='PERSONA'?text('人格指令','Persona instructions'):text('知识内容','Knowledge content')}}<textarea class="content-editor knowledge-editor" v-model="knowledgeForm.content" required></textarea></label></div><div class="editor-actions"><label class="file-button">{{text('导入 Word / PDF / Markdown','Import Word / PDF / Markdown')}}<input type="file" accept=".doc,.docx,.pdf,.md,.txt" @change="importKnowledge"></label><button>{{text('保存','Save')}}</button><small>{{adminNotice}}</small></div></form>
    <div class="admin-list knowledge-list"><article v-for="item in adminKnowledge" :key="item.id" :class="{disabled:!item.enabled}"><span :class="['knowledge-kind',item.kind.toLowerCase()]">{{item.kind==='PERSONA'?text('人格','Persona'):text('知识','Knowledge')}}</span><div><small>{{item.enabled?text('已启用','Enabled'):text('已停用','Disabled')}} · {{item.tags}}</small><h3>{{item.title}}</h3><p>{{item.content.slice(0,120)}}{{item.content.length>120?'…':''}}</p></div><button :class="['toggle-entry',{off:!item.enabled}]" @click="toggleKnowledge(item)"><i></i>{{item.enabled?text('停用','Disable'):text('启用','Enable')}}</button><button @click="editKnowledge(item)">{{text('编辑','Edit')}}</button><button class="danger" @click="removeKnowledge(item.id)">{{text('删除','Delete')}}</button></article></div>
  </div>
