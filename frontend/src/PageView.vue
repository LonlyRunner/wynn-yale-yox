...
async function loadAdmin(){if(!sessionStorage.getItem('wynn-auth'))return;try{[adminPosts.value,adminJournals.value,adminComments.value,adminJobs.value,adminKnowledge.value]=await Promise.all([api<AdminPost[]>('/admin/posts'),api<JournalItem[]>('/admin/journals'),api<AdminComment[]>('/admin/comments'),api<AiJob[]>('/admin/ai/jobs'),api<AdminKnowledge[]>('/admin/knowledge')])}catch{}}
...
<main :class="['site',{dark}]">
  ...
  <template v-else-if="route.name==='gallery'"><section class="page-head"><small>VISUAL ARCHIVE</small><h1>{{text('影像收藏','Visual Archive')}}</h1><p>{{text('点击照片即可翻转，查看并复制它的创作提示词。','Tap a photo to flip it, then view and copy its creation prompt.')}}</p></section><section class="gallery user-gallery"><figure v-for="(item,i) in galleryItems" :key="item.id||item.url" :class="{'gallery-portrait':i%5===4,flipped:flippedGallery===galleryKey(item,i)}" tabindex="0" @click="flipGallery(item,i)" @keyup.enter="flipGallery(item,i)"><div class="gallery-flip"><div class="gallery-face gallery-front"><img :src="previewImageUrl(item,1200)" :alt="text(item.titleZh,item.titleEn)" loading="lazy" decoding="async" @error="restoreOriginalImage($event,item.url)"><figcaption>{{text(item.titleZh,item.titleEn)}} · {{text('点击翻转','FLIP')}}</figcaption></div><div class="gallery-face gallery-back" @click.stop="flippedGallery=undefined"><small>IMAGE PROMPT · {{String(i+1).padStart(2,'0')}}</small><h2>{{text(item.titleZh,item.titleEn)}}</h2><p>{{text(item.promptZh||'正在生成提示词…',item.promptEn||'Generating prompt…')}}</p><button @click.stop="copyGalleryPrompt(item,i)">{{copiedGallery===galleryKey(item,i)?text('已复制 ✓','Copied ✓'):text('复制提示词','Copy prompt')}}</button><span>{{text('点击空白处返回照片','Tap elsewhere to return')}}</span></div></div></figure></section></template>
