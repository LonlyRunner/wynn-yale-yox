...
const pelicanModels=['DeepSeek','Qwen','LLAPI Relay'] as const
const pelicanModel=ref<(typeof pelicanModels)[number]>('DeepSeek'),pelicanJumping=ref(false)
...
watch(()=>route.fullPath,()=>{mobileOpen.value=false;notice.value='';soundOn.value=false;clearInterval(aiPollTimer);if(route.name==='game')resetGame();else{playing.value=false;clearInterval(timer.value)}loadPublic();if(route.name==='admin')loadAdmin()})
</script>
  ...
  <section v-else-if="route.name==='game'" class="game-page">
    ...
    <div class="game-shell">
      <div v-if="gameId==='pelican'" class="pelican-panel">
        <div class="model-switcher"><span>{{text('选择画风模型','Choose art model')}}</span><button v-for="model in pelicanModels" :key="model" :class="{active:pelicanModel===model}" @click="pelicanModel=model">{{model}}</button></div>
        <div :class="['pelican-ride',{'is-playing':playing,'is-jumping':pelicanJumping},`art-${pelicanModels.indexOf(pelicanModel)}`]" @click="jumpPelican">
          <svg viewBox="0 0 1000 520" role="img" :aria-label="text('鹈鹕骑自行车的二维动画','2D animation of a pelican riding a bicycle')">
          ...
          <div class="ride-status"><b>{{pelicanModel}}</b><span>{{text('点击画面或按空格跳跃','Tap the scene or press Space to jump')}}</span></div>
      ...
      <div v-else class="stack-wrap"><div :class="['stack',`move-${stackDirection}`]"><button v-for="(cell,i) in board" :key="`${i}-${stackMoveTick}`" :data-value="cell||0" :class="{filled:cell}">{{cell||''}}</button></div><div class="stack-controls"><button @click="stackMove('up')">↑</button><span><button @click="stackMove('left')">←</button><button @click="stackMove('down')">↓</button><button @click="stackMove('right')">→</button></span></div></div>
      <aside class="game-guide"><small>HOW TO PLAY</small><h2>{{text('玩法说明','How to play')}}</h2><ol v-if="gameId==='pelican'"><li>{{text('选择 DeepSeek、Qwen 或 LLAPI Relay 查看不同配色画风。','Choose DeepSeek, Qwen, or LLAPI Relay for a different palette.')}}</li><li>{{text('点击“开始骑行”，再点击画面或按空格让鹈鹕跳跃。','Start the ride, then tap the scene or press Space to jump.')}}</li><li>{{text('持续骑行会累积分数。','Keep riding to build your score.')}}</li></ol><ol v-else-if="gameId==='memory'"><li>{{text('每次翻开两张牌，图标相同即可配对。','Flip two cards; matching icons stay open.')}}</li><li>{{text('完成全部配对后自动进入下一关。','Match every pair to advance automatically.')}}</li><li>{{text('每关增加两组卡片，得分倍率同步提升。','Each level adds two pairs and raises the score multiplier.')}}</li></ol><ol v-else><li>{{text('使用方向键或下方按钮移动全部方块。','Use arrow keys or the controls to move all tiles.')}}</li><li>{{text('相同数字碰撞后合并，目标是 2048。','Equal tiles merge; your target is 2048.')}}</li><li>{{text('每次有效移动都会出现一个新方块。','A new tile appears after every valid move.')}}</li></ol><details v-if="gameId==='pelican'"><summary>{{text('本动画使用的提示词','Prompt used for this animation')}}</summary><code>创建一个HTML，内容是SVG绘制一个鹈鹕骑自行车的2D动画。</code></details><a v-if="gameId==='pelican'" class="demo-link" href="/demos/pelican-bicycle.html" target="_blank">↗ {{text('打开独立 HTML 动画','Open standalone HTML animation')}}</a><a v-if="gameId==='pelican'" class="llapi-promo" href="https://llapi.org" target="_blank" rel="noreferrer"><b>灵链云 API · llapi.org</b><span>{{text('一个入口连接多种 AI 模型，去看看 ↗','One endpoint for multiple AI models. Explore ↗')}}</span></a><RouterLink class="game-return-bottom" to="/games">← {{text('返回全部游戏','Back to all games')}}</RouterLink></aside>
    </div>
  </section>
