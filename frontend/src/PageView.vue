...
function refreshChatVoices(){const voices=window.speechSynthesis?.getVoices()||[];chatVoices.value=[...voices].sort((a,b)=>{const preferred=(voice:SpeechSynthesisVoice)=>/^(zh|en)/i.test(voice.lang)?0:1;return preferred(a)-preferred(b)||a.lang.localeCompare(b.lang)||a.name.localeCompare(b.name)});if(chatVoiceName.value&&!voices.some(voice=>voice.name===chatVoiceName.value))chatVoiceName.value=''}
