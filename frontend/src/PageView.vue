...
type GalleryItem = { id?:number; titleZh:string; titleEn:string; mediaType:string; url:string }
type AiJob = { id:number; provider:string; type:string; prompt:string; status:string; resultUrl:string; error:string; resolution?:string; duration?:number; createdAt?:string }
...
const fallbackGallery:GalleryItem[] = [
  ['绯色旅人','Crimson Traveler','fantasy-red-01.png'],['花灯入梦','Lantern Dream','fantasy-peach-01.png'],['风起长歌','Song in the Wind','fantasy-red-02.png'],['琉璃碎光','Crystal Light','fantasy-silver-01.png'],['夜蓝华章','Midnight Blue','fantasy-blue.png'],['春灯微醺','Spring Lanterns','fantasy-peach-02.png'],['回望','Looking Back','fantasy-red-03.png'],['灯影','Lantern Glow','fantasy-peach-03.png'],['旅者','Traveler','profile-anime-01.png'],['黑金侧影','Black & Gold','profile-anime-02.png'],['小小绅士','Little Gentleman','profile-chibi-01.png'],['静坐片刻','A Quiet Moment','profile-chibi-02.png'],['粉色飞行员','Pink Aviator','avatar-pink.jpg'],['白色回忆','White Memory','avatar-white.jpg']
].map(([titleZh,titleEn,file])=>({titleZh,titleEn,mediaType:'IMAGE',url:`/media/user-gallery/${file}`}))
...
async function loadPublic(){loadPosts();api<Profile>('/public/profile').then(v=>profile.value=v).catch(()=>{});api<GalleryItem[]>('/public/media').then(v=>{if(v.length)galleryItems.value=v}).catch(()=>{});if(route.name==='studio')loadQuota();if(route.name==='article')loadArticle()}
...
async function uploadGallery(event:Event){const files=Array.from((event.target as HTMLInputElement).files||[]);for(const [index,file] of files.entries()){const key=`gallery/${Date.now()}-${index}-${file.name.replace(/[^a-zA-Z0-9._-]/g,'-')}`;const signed=await api<{url:string}>('/admin/oss/upload-url',{method:'POST',body:JSON.stringify({objectKey:key,contentType:file.type})});await fetch(signed.url,{method:'PUT',headers:{'Content-Type':file.type},body:file});await api('/admin/media',{method:'POST',body:JSON.stringify({objectKey:key,titleZh:file.name.replace(/\.[^.]+$/,''),titleEn:file.name.replace(/\.[^.]+$/,''),mediaType:'IMAGE',sortOrder:galleryItems.value.length+index})})}adminNotice.value=text('图片已上传至 OSS','Images uploaded to OSS');await loadPublic()}
...
<main :class="['site',{dark}]">
  ...
  <template v-else-if="route.name==='blog'"><section class="page-head"><small>NOTES & EXPERIMENTS</small><h1>{{text('技术与思考','Technology & Thoughts')}}</h1><p>{{text('记录 Java、AI 工程与产品实践，也记录那些仍在形成中的判断。','Notes on Java, AI engineering, products, and ideas still taking shape.')}}</p><form class="blog-search" @submit.prevent="searchPosts"><input v-model="blogSearch" :placeholder="text('搜索文章、标签或内容','Search posts, tags, or content')"><button>{{text('搜索','Search')}}</button><a href="/api/public/rss.xml" target="_blank">RSS ↗</a></form></section><section v-if="posts.length" class="blog-grid"><RouterLink class="feature" :style="posts[0].coverUrl?{backgroundImage:`linear-gradient(transparent 30%,rgba(3,8,10,.92)),url(${posts[0].coverUrl})`}:{}" :to="`/blog/${posts[0].slug}`"><div><small>{{posts[0].tag}}</small><h2>{{text(posts[0].zh,posts[0].en)}}</h2><span>{{posts[0].date}} · 12 MIN</span></div></RouterLink><div class="post-list"><RouterLink v-for="(post,i) in posts.slice(1)" :key="post.slug" class="post" :to="`/blog/${post.slug}`"><div><small>{{post.tag}}</small><h2>{{text(post.zh,post.en)}}</h2><p>{{text(post.summaryZh||'',post.summaryEn||'')}}</p><span>{{post.date}}</span></div><b>0{{i+1}}</b></RouterLink></div></section><p v-else class="empty-state">{{text('没有找到文章','No posts found')}}</p></template>
