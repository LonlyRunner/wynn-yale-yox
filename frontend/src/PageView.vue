...
type AiJob = { id:number; provider:string; model?:string; type:string; prompt:string; status:string; resultUrl:string; downloadUrl?:string; error:string; resolution?:string; duration?:number; createdAt?:string; inGallery?:boolean }
type GenerationModel = { id:string; name:string; type:'IMAGE'|'VIDEO'; model:string }
...
const chatAttachments=ref<File[]>([]), chatFileInput=ref<HTMLInputElement>(), chatListening=ref(false), chatHistoryLoaded=ref(false)
const chatAutoSpeak=ref(localStorage.getItem('wynn-chat-speak')==='1')
const chatOwner=ref(sessionStorage.getItem('wynn-auth')==='1')
const chatSpeechAvailable='speechSynthesis' in window
const chatVoices=ref<SpeechSynthesisVoice[]>([]), chatVoiceName=ref(localStorage.getItem('wynn-chat-voice')||'')
...
function refreshChatVoices(){
  const voices=window.speechSynthesis?.getVoices()||[]
  if(!voices.length){chatVoices.value=[];chatVoiceName.value='';return}
  const chinese=voices.filter(voice=>/^zh/i.test(voice.lang)||/Chinese|中文|普通话/i.test(voice.name))
  const cute=/Yaoyao|Xiaoxiao|Xiaoyi|Xiaomeng|Xiaobei|Yunxia|Huihui|Tingting|Meijia|Sinji|Sin-ji|瑶瑶|晓晓|小艺|小梦|小贝|云霞|慧慧|婷婷|美佳/i
  const male=/Kangkang|Yunxi|Yunjian|Yunyang|Yunhao|Danny|康康|云希|云健|云扬|云皓/i
  const preferred=chinese.filter(voice=>cute.test(voice.name)&&!male.test(voice.name))
  chatVoices.value=(preferred.length?preferred:chinese.filter(voice=>!male.test(voice.name))).sort((a,b)=>a.name.localeCompare(b.name))
  if(!chatVoiceName.value)chatVoiceName.value=localStorage.getItem('wynn-chat-voice')||''
  if(chatVoiceName.value&&!chatVoices.value.some(voice=>voice.name===chatVoiceName.value))chatVoiceName.value=''
  if(!chatVoiceName.value&&chatVoices.value.length){chatVoiceName.value=chatVoices.value[0].name;selectChatVoice()}
}
function selectChatVoice(){localStorage.setItem('wynn-chat-voice',chatVoiceName.value)}
async function refreshChatOwner(){try{const me=await api<{authenticated:boolean}>('/auth/me');chatOwner.value=me.authenticated;if(me.authenticated)sessionStorage.setItem('wynn-auth','1');else sessionStorage.removeItem('wynn-auth')}catch{chatOwner.value=false}}
async function loadChatHistory(){if(chatHistoryLoaded.value)return;await refreshChatOwner();if(!chatOwner.value)return;chatHistoryLoaded.value=true;try{const items=await api<ChatMessage[]>('/chat/history');if(items.length)chatMessages.value=items}catch(error){chatHistoryLoaded.value=false;chatError.value=aiError(error)}finally{scrollChat()}}
function openCatChat(){chatOpen.value=true;chatError.value='';refreshChatVoices();setTimeout(refreshChatVoices,500);if(!chatMessages.value.length)chatMessages.value.push({role:'assistant',content:text('你好呀，我是团子 🐾 可以陪你聊天，也能看图片、读取文档和听语音。','Hi, I’m Tuanzi 🐾 I can chat, inspect images, read documents, and listen to audio.')});loadChatModels();loadChatHistory();scrollChat()}
function selectChatFiles(event:Event){const input=event.target as HTMLInputElement;const selected=Array.from(input.files||[]);const valid=selected.filter(file=>file.size<=12*1024*1024);chatAttachments.value=[...chatAttachments.value,...valid].slice(0,5);if(valid.length!==selected.length)chatError.value=text('单个附件不能超过 12MB','Each attachment must be 12MB or smaller');input.value=''}
function removeChatAttachment(index:number){chatAttachments.value.splice(index,1)}
function listenChat(){
  ...
  if(!SpeechRecognition){chatError.value=text('当前浏览器不支持语音听写，可以上传语音文件','This browser does not support dictation; upload an audio file instead');return}
  const recognition=new SpeechRecognition();recognition.lang=lang.value==='zh'?'zh-CN':'en-US';recognition.interimResults=false;chatListening.value=true
  recognition.onresult=(event:any)=>{const transcript=event.results?.[0]?.[0]?.transcript||'';chatInput.value=`${chatInput.value}${chatInput.value?' ':''}${transcript}`}
  recognition.onerror=()=>chatError.value=text('没有识别到语音，请重试','Speech was not recognized; try again')
  recognition.onend=()=>chatListening.value=false;recognition.start()
}
function speakChat(value:string){if(!('speechSynthesis' in window))return;window.speechSynthesis.cancel();const speech=new SpeechSynthesisUtterance(value.replace(/[#*_`>\[\]()]/g,' '));const selected=chatVoices.value.find(voice=>voice.name===chatVoiceName.value);if(selected){speech.voice=selected;speech.lang=selected.lang}else speech.lang=lang.value==='zh'?'zh-CN':'en-US';speech.rate=.98;window.speechSynthesis.speak(speech)}
function toggleAutoSpeak(){chatAutoSpeak.value=!chatAutoSpeak.value;localStorage.setItem('wynn-chat-speak',chatAutoSpeak.value?'1':'0');if(!chatAutoSpeak.value)window.speechSynthesis?.cancel()}
async function clearChatHistory(){try{await api('/chat/history',{method:'DELETE'});chatMessages.value=[];chatHistoryLoaded.value=true;openCatChat()}catch(error){chatError.value=aiError(error)}}
async function sendCatMessage(){const message=chatInput.value.trim(),files=[...chatAttachments.value];if((!message&&!files.length)||chatBusy.value)return;const history=chatMessages.value.map(item=>({role:item.role,content:item.content})).slice(-10);const shown=[message,...files.map(file=>`📎 ${file.name}`)].filter(Boolean).join('\n');chatMessages.value.push({role:'user',content:shown});chatInput.value='';chatAttachments.value=[];chatError.value='';chatBusy.value=true;scrollChat();try{let reply:{answer:string;model:string;sources:Array<{type:string;title:string}>};if(files.length){const form=new FormData();form.append('model',chatModel.value);form.append('message',message);form.append('historyJson',JSON.stringify(history));files.forEach(file=>form.append('files',file));reply=await api('/chat/messages/multimodal',{method:'POST',body:form})}else reply=await api('/chat/messages',{method:'POST',body:JSON.stringify({model:chatModel.value,message,history})});chatMessages.value.push({role:'assistant',content:reply.answer,sources:reply.sources});if(chatAutoSpeak.value)speakChat(reply.answer);scrollChat()}catch(error){chatError.value=aiError(error)}finally{chatBusy.value=false}}
...
const previewMedia=ref<GalleryItem>()
...
const studioModels=ref<GenerationModel[]>([]), studioModelsLoading=ref(false), studioModelsError=ref('')
const chosenModels=ref({IMAGE:localStorage.getItem('wynn-studio-image-model')||'',VIDEO:localStorage.getItem('wynn-studio-video-model')||''})
const studioModel=computed({get:()=>chosenModels.value[creatorMode.value],set:(id:string)=>{chosenModels.value[creatorMode.value]=id;localStorage.setItem(`wynn-studio-${creatorMode.value.toLowerCase()}-model`,id)}})
const availableStudioModels=computed(()=>studioModels.value.filter(model=>model.type===creatorMode.value))
const selectedStudioModel=computed(()=>availableStudioModels.value.find(model=>model.id===studioModel.value))
async function loadStudioModels(){
  if(studioModelsLoading.value)return
  studioModelsLoading.value=true;studioModelsError.value=''
  try{
    studioModels.value=await api<GenerationModel[]>('/ai/models')
    for(const type of ['IMAGE','VIDEO'] as const){
      const choices=studioModels.value.filter(model=>model.type===type)
      if(!choices.some(model=>model.id===chosenModels.value[type]))chosenModels.value[type]=choices[0]?.id||''
    }
  }catch{studioModels.value=[];studioModelsError.value=text('模型列表读取失败，请重试。','Could not load models. Please retry.')}
  finally{studioModelsLoading.value=false}
}
...
const studioBusy=computed(()=>studioSubmitting.value||studioLoading.value||studioModelsLoading.value||pendingJob(currentJob.value))
...
async function generate(){
  if(studioBusy.value||!selectedStudioModel.value||!prompt.value.trim())return
  ...
  try{
    const job=await api<AiJob>('/ai/jobs',{method:'POST',body:JSON.stringify({type:creatorMode.value,model:studioModel.value,prompt:prompt.value,aspectRatio:aspectRatio.value,resolution:resolution.value,duration:duration.value})})
    if(version!==aiViewVersion||route.name!=='studio')return
...
const adminTab=ref<'overview'|'posts'|'journals'|'media'|'ai'|'knowledge'|'comments'|'guestbook'>('overview'), adminPosts=ref<AdminPost[]>([]), adminJournals=ref<JournalItem[]>([]), adminComments=ref<AdminComment[]>([]), adminJobs=ref<AiJob[]>([]), adminKnowledge=ref<AdminKnowledge[]>([])
...
async function addJobToGallery(job:AiJob){try{await api(`/admin/ai/jobs/${job.id}/gallery`,{method:'POST'});job.inGallery=true;adminNotice.value=text('图片已加入影像库','Image added to gallery');await Promise.all([loadAdmin(),loadPublic()])}catch(error){adminNotice.value=aiError(error)}}
async function removeMedia(item:GalleryItem){if(!item.id)return;try{await api(`/admin/media/${item.id}`,{method:'DELETE'});galleryItems.value=galleryItems.value.filter(media=>media.id!==item.id);if(previewMedia.value?.id===item.id)previewMedia.value=undefined;adminNotice.value=text('图片已删除','Image deleted');await Promise.all([loadPublic(),loadAdmin()])}catch(error){adminNotice.value=aiError(error)}}
...
<main :class="['site',{dark}]">
  <header v-if="route.name!=='admin'" class="topbar">
    ...
    <nav :class="{open:mobileOpen}">
      ...
      <RouterLink to="/share"><img class="nav-charm" src="/media/cat-items/fish-two.webp" alt="">{{text('分享','Links')}}</RouterLink>
      <RouterLink to="/guestbook"><img class="nav-charm" src="/media/fluffy-kitten.webp" alt="">{{text('留言板','Guestbook')}}</RouterLink>
  ...
  <Guestbook v-else-if="route.name==='guestbook'" :lang="lang" />

  <section v-else-if="route.name==='share'" class="share-page">
    <div class="share-intro"><small>SHARED LINKS · WYNN YALE YOX</small><h1>{{text('分享一些网站','A few links to share')}}</h1><p>{{text('把常用的入口放在一起，想用的时候可以直接找到。','A small collection of handy places, all in one spot.')}}</p><img class="share-intro-charm" src="/media/cat-items/yarn.webp" alt="" aria-hidden="true"></div>
    <div class="share-grid">
      <article class="share-card"><div class="share-card-meta"><img src="/media/cat-items/food.webp" alt="" aria-hidden="true"><small>01 / MODEL API</small></div><h2>{{text('火锅中转站','Huoguo API relay')}}</h2><p>{{text('这里提供多种主流 AI 模型的 API 中转服务，可以在一个平台挑选需要的模型。价格比较实惠，也有售后支持；具体模型、价格和使用规则以平台说明为准。','Access APIs for many popular AI models through one relay platform. It offers affordable pricing and support; check the provider for current models, prices, and terms.')}}</p><a href="https://huoguo.cc.cd/register?aff=MK3KP5KD7DXJ" target="_blank" rel="noopener noreferrer sponsored">{{text('打开火锅中转站','Open Huoguo')}} ↗</a></article>
      <article class="share-card"><div class="share-card-meta"><img src="/media/cat-items/collar.webp" alt="" aria-hidden="true"><small>02 / NETWORK</small></div><h2>{{text('魔戒','Mojie')}}</h2><p>{{text('想睁眼看世界，访问 Twitter、YouTube、TikTok，可以看看这个网络工具。下面是同一网站的两个注册入口，任选其一。','Explore the wider world and visit Twitter, YouTube, or TikTok. Both links below lead to the same network service; choose either one.')}}</p><p class="share-help">{{text('只注册不配置，大概率无法正常使用，还需要安装客户端并设置。如果无法使用，','Signing up alone usually will not get it working; you also need to install and configure a client. If it does not work,')}} <RouterLink to="/about">{{text('联系我，我教你设置 →','contact me for setup help →')}}</RouterLink></p><div class="share-actions"><a href="https://mojie.co/register?aff=dtwKRNVc" target="_blank" rel="noopener noreferrer sponsored">{{text('入口一','Link one')}} ↗</a><a href="https://mojie.co/register?aff=jr9Cy7pz" target="_blank" rel="noopener noreferrer sponsored">{{text('入口二','Link two')}} ↗</a></div></article>
      <article class="share-card"><div class="share-card-meta"><img src="/media/cat-items/paw.webp" alt="" aria-hidden="true"><small>03 / DESIGN</small></div><h2>MotionSites</h2><p>{{text('收集 AI 网页与应用设计提示词、模板和页面区块，涵盖落地页、作品集、SaaS 等方向。可以按类型找灵感、复制提示词，再用 AI 工具起稿。','A library of AI design prompts, templates, and reusable sections for sites and apps. Browse landing pages, portfolios, SaaS examples, and more, then copy a prompt to start designing.')}}</p><a href="https://motionsites.org/" target="_blank" rel="noopener noreferrer">{{text('浏览设计灵感','Explore designs')}} ↗</a></article>
      <article class="share-card"><div class="share-card-meta"><img src="/media/cat-items/mouse.webp" alt="" aria-hidden="true"><small>04 / SOFTWARE</small></div><h2>{{text('阿酷杂货铺','Coolexe')}}</h2><p>{{text('整理软件、App 和技术教程，按办公、系统工具、媒体工具、编程开发等类别查找。适合找日常工具、阅读功能介绍和安装资料；具体下载条件以站内页面为准。','A curated software and tech resource site. Browse office, system, media, and development tools for introductions and how-to guides; check each page for download conditions.')}}</p><a href="https://www.coolexe.com/?from=adzhp.cn" target="_blank" rel="noopener noreferrer">{{text('打开阿酷杂货铺','Open Coolexe')}} ↗</a></article>
      <article class="share-card"><div class="share-card-meta"><img src="/media/cat-items/yarn.webp" alt="" aria-hidden="true"><small>05 / SOFTWARE</small></div><h2>{{text('软仓','RuanCang')}}</h2><p>{{text('按 Office、Adobe、CAD、三维建模、图像设计和影音等类别收录软件资源与安装教程，方便按专业方向查找。部分资源为非官方修改版，下载前请核对来源与授权。','A categorized catalog of software resources and installation guides, including Office, Adobe, CAD, 3D, design, and media tools. Some packages are modified unofficial versions; check their source and license before downloading.')}}</p><a href="https://www.ruancang.net/#/" target="_blank" rel="noopener noreferrer">{{text('查看软仓目录','Browse RuanCang')}} ↗</a></article>
      <article class="share-card"><div class="share-card-meta"><img src="/media/cat-items/fish-two.webp" alt="" aria-hidden="true"><small>06 / SEARCH</small></div><h2>{{text('海搜','HaiSou')}}</h2><p>{{text('搜索第三方云盘分享信息，可以按标题或文件名检索，并筛选阿里、百度、夸克、迅雷等云盘。它提供搜索结果与跳转入口，不托管实际文件。','Search third-party cloud-drive shares by title or file name, with filters for Aliyun, Baidu, Quark, Xunlei, and more. It indexes links and does not host the files.')}}</p><a href="https://haisou.cc/" target="_blank" rel="noopener noreferrer">{{text('前往海搜','Open HaiSou')}} ↗</a></article>
    </div>
    <p class="share-note">{{text('部分链接含邀请或来源参数。点击后会离开本站；具体服务内容与规则请以对应网站为准。','Some links include referral or source parameters. You will leave this site; please check each provider for its current service details and terms.')}}</p>
  </section>
  ...
  <section v-else-if="route.name==='studio'" class="studio">
    ...
    <div class="studio-main">
      ...
      <div class="studio-options"><label class="studio-model">{{text(creatorMode==='IMAGE'?'图片模型':'视频模型',creatorMode==='IMAGE'?'Image model':'Video model')}}<select v-model="studioModel" :disabled="studioBusy||!availableStudioModels.length"><option value="" disabled>{{studioModelsLoading?text('正在读取模型…','Loading models…'):text('暂无可用模型','No available models')}}</option><option v-for="model in availableStudioModels" :key="model.id" :value="model.id">{{model.id==='auto'?text('自动选择（失败时回退千问）','Automatic (Qwen fallback)'):model.name}}</option></select></label><label>{{text('画面比例','Aspect ratio')}}<select v-model="aspectRatio"><option>16:9</option><option>9:16</option><option>1:1</option><option>4:3</option><option>3:4</option></select></label><label v-if="creatorMode==='VIDEO'">{{text('清晰度','Resolution')}}<select v-model="resolution"><option>480p</option><option>720p</option><option>1080p</option></select></label><label v-if="creatorMode==='VIDEO'">{{text('时长','Duration')}}<input v-model.number="duration" type="number" min="1" max="15"><span>s</span></label></div>
      <p v-if="studioModelsError" class="studio-model-note" role="alert">{{studioModelsError}} <button type="button" @click="loadStudioModels">{{text('重试','Retry')}}</button></p><p v-else-if="!studioModelsLoading" class="studio-model-note">{{!selectedStudioModel?text('当前未配置可用模型，请联系管理员。','No models configured. Contact the administrator.'):studioModel==='auto'?text('优先使用默认图片模型，失败时尝试千问。','Uses the default image model, with Qwen as fallback.'):text('按所选模型生成；失败时可手动切换其他模型重试。','Uses your selected model. If it fails, choose another model to retry.')}}</p>
      <div class="prompt"><textarea v-model="prompt" :placeholder="text('描述你想生成的内容','Describe what you want to create')"></textarea><footer><span>{{aspectRatio}} · {{creatorMode==='VIDEO'?`${resolution} · ${duration}s`:'AI IMAGE'}}</span><button @click="generate" :disabled="studioBusy||!selectedStudioModel||!prompt.trim()||(!quota.owner&&creatorMode==='IMAGE'&&!quota.imageRemaining)||(!quota.owner&&creatorMode==='VIDEO'&&!quota.videoRemaining)">{{studioBusy?text('处理中…','Processing…'):text('开始生成 ↗','Generate ↗')}}</button></footer></div>
      <div class="studio-history">
        <label v-if="aiHistory.length">{{text('最近生成记录','Recent generations')}}<select :value="currentJob?.id||''" :disabled="studioSubmitting||studioLoading" @change="selectStudioJob(Number(($event.target as HTMLSelectElement).value))"><option value="" disabled>{{text('请选择记录','Choose a result')}}</option><option v-for="job in aiHistory" :key="job.id" :value="job.id">#{{job.id}} · {{job.model?job.model+' · ':''}}{{job.prompt.slice(0,48)}}</option></select></label>
      ...
      <div class="result generated-result" :aria-busy="studioBusy">
        ...
        <small v-if="currentJob?.model">{{text('使用模型：','Model: ')}}{{currentJob.model}}{{currentJob.provider==='qwen-fallback'?text('（自动回退）',' (automatic fallback)'):''}}</small>
  ...
  <div class="admin-card"><h2>{{text('上传影像到 OSS','Upload images to OSS')}}</h2><p>{{text('支持一次选择多张图片，并显示每张图片的上传与 AI 解析进度。','Choose multiple images and track each upload and AI analysis step.')}}</p><label class="file-button">{{text('选择图片','Choose images')}}<input type="file" accept="image/*" multiple @change="uploadGallery"></label><small>{{adminNotice}}</small>
    ...
    <div class="media-admin-grid"><article v-for="item in galleryItems" :key="item.url"><button class="media-preview" type="button" @click="previewMedia=item"><img :src="previewImageUrl(item,480)" :alt="item.titleZh" loading="lazy" decoding="async" @error="restoreOriginalImage($event,item.url)"><span>⌕ {{text('放大预览','Preview')}}</span></button><footer><b>{{text(item.titleZh,item.titleEn)}}</b><button class="danger" type="button" @click="removeMedia(item)">{{text('删除','Delete')}}</button></footer></article></div>
  </div>
</template><template v-else-if="adminTab==='ai'"><div v-if="!adminJobs.length" class="admin-empty"><span>✦</span><h3>{{text('还没有 AI 生成记录','No AI generation records yet')}}</h3><p>{{text('在 AI 创作室提交图片或视频任务后，记录会自动显示在这里。','Image and video jobs submitted in AI Studio will appear here automatically.')}}</p></div><div v-else class="admin-list"><article v-for="job in adminJobs" :key="job.id"><div><small>{{job.type}} · {{job.model||job.provider}} · {{job.status}}</small><h3>{{job.prompt}}</h3><a v-if="job.resultUrl" :href="job.resultUrl" target="_blank">{{text('查看结果','View result')}} ↗</a><a v-if="job.downloadUrl" class="admin-download" :href="job.downloadUrl" download>↓ {{text('下载文件','Download')}}</a><p v-if="job.error">{{job.error}}</p></div><button v-if="job.type==='IMAGE'&&job.status==='COMPLETED'" :disabled="job.inGallery" @click="addJobToGallery(job)">{{job.inGallery?text('已加入影像库','In gallery'):text('加入影像库','Add to gallery')}}</button><button class="danger" @click="removeJob(job.id)">{{text('删除','Delete')}}</button></article></div></template><template v-else-if="adminTab==='knowledge'">
  ...
  <section v-if="chatOpen" class="cat-chat" role="dialog" aria-modal="false" :aria-label="text('团子聊天助手','Tuanzi chat assistant')">
    ...
    <div class="cat-chat-toolbar"><label>{{text('对话模型','Model')}}<select v-model="chatModel"><option v-for="model in chatModels" :key="model.id" :value="model.id">{{model.name}}</option></select></label><label class="chat-voice-select">{{text('朗读音色','Voice')}}<select v-model="chatVoiceName" :disabled="!chatSpeechAvailable" @focus="refreshChatVoices" @change="selectChatVoice"><option value="">{{!chatSpeechAvailable?text('当前浏览器不支持朗读','Speech unavailable in this browser'):chatVoices.length?text('系统默认音色','System default'):text('系统默认（暂无可选女声）','System default (no other voice available)')}}</option><option v-for="voice in chatVoices" :key="`${voice.name}-${voice.lang}`" :value="voice.name">{{voice.name}} · {{voice.lang}}</option></select></label><button type="button" :class="{active:chatAutoSpeak}" :disabled="!chatSpeechAvailable" @click="toggleAutoSpeak">🔊 {{text('自动朗读','Auto voice')}}</button><button v-if="chatOwner" type="button" @click="clearChatHistory">{{text('清空记录','Clear history')}}</button></div>
    <div class="cat-chat-messages"><article v-for="(message,index) in chatMessages" :key="index" :class="message.role"><span>{{message.role==='assistant'?'🐾':'你'}}</span><div><div class="chat-content" v-html="markdown(message.content)"></div><button v-if="message.role==='assistant'&&chatSpeechAvailable" class="chat-speak" type="button" @click="speakChat(message.content)">🔈 {{text('朗读','Read')}}</button><small v-if="message.sources?.length">{{text('参考：','Sources: ')}}{{message.sources.map(source=>source.title).join(' · ')}}</small></div></article><article v-if="chatBusy" class="assistant thinking"><span>🐾</span><div><p>{{text('团子正在理解附件并翻知识库…','Tuanzi is reading attachments and searching the knowledge base…')}}</p></div></article></div>
    <div v-if="chatAttachments.length" class="chat-attachments"><button v-for="(file,index) in chatAttachments" :key="`${file.name}-${index}`" type="button" @click="removeChatAttachment(index)">📎 {{file.name}} <b>×</b></button></div>
    <form @submit.prevent="sendCatMessage"><div class="chat-input-tools"><button type="button" :title="text('添加照片、语音或文件','Attach photo, audio, or file')" @click="chatFileInput?.click()">＋</button><button type="button" :class="{active:chatListening}" :title="text('语音输入','Voice input')" @click="listenChat">🎙</button><input ref="chatFileInput" type="file" multiple accept="image/*,audio/*,.pdf,.doc,.docx,.txt,.md,.csv" @change="selectChatFiles"></div><textarea v-model="chatInput" maxlength="2000" rows="2" :placeholder="text('发消息，或添加照片、语音、文档…','Send a message, photo, audio, or document…')" @keydown.enter.exact.prevent="sendCatMessage"></textarea><button class="chat-send" :disabled="chatBusy||(!chatInput.trim()&&!chatAttachments.length)">{{text('发送','Send')}} →</button></form><p v-if="chatError" class="cat-chat-error">{{chatError}}</p>
  </section>
  <div v-if="previewMedia" class="media-lightbox" role="dialog" aria-modal="true" @click.self="previewMedia=undefined"><button type="button" :aria-label="text('关闭预览','Close preview')" @click="previewMedia=undefined">×</button><figure><img :src="previewMedia.url" :alt="text(previewMedia.titleZh,previewMedia.titleEn)"><figcaption><b>{{text(previewMedia.titleZh,previewMedia.titleEn)}}</b><p>{{text(previewMedia.promptZh||'',previewMedia.promptEn||'')}}</p></figcaption></figure></div>
</main>
</template>
...
.share-page { min-height: calc(100vh - 76px); padding: 78px 6vw 100px; color: #5b4038; background: radial-gradient(circle at 88% 14%, #f8dcd2 0, transparent 26%), linear-gradient(145deg, #fff9f3, #f9ede7); }
.share-intro { position: relative; max-width: 1100px; margin: 0 auto 45px; }
.share-intro small { color: #b87869; font-size: 11px; letter-spacing: .22em; }
.share-intro h1 { max-width: 720px; margin: 16px 0; font: 500 clamp(44px, 5.5vw, 76px)/1.12 var(--serif); }
.share-intro p { max-width: 600px; margin: 0; color: #8c7167; line-height: 1.8; }
.share-intro-charm { position: absolute; right: 3%; top: -18px; width: 140px; height: 140px; object-fit: contain; filter: drop-shadow(0 12px 18px #ad756b30); transform: rotate(-12deg); }
.share-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 22px; max-width: 1100px; margin: auto; }
.share-card { display: flex; min-width: 0; min-height: 320px; flex-direction: column; align-items: flex-start; padding: 34px; border: 1px solid #ebd0c3; border-radius: 28px; background: #fffaf5; box-shadow: 0 18px 42px rgb(106 70 57 / 8%); }
.share-card:nth-child(even) { background: #f8e5dc; }
.share-card-meta { display: flex; width: 100%; align-items: center; justify-content: space-between; }
.share-card-meta img { width: 65px; height: 65px; object-fit: contain; filter: drop-shadow(0 7px 9px #ab766530); }
.share-card-meta small { color: #aa7d70; font-size: 10px; letter-spacing: .2em; }
.share-card h2 { margin: 31px 0 8px; font: 36px/1.2 var(--serif); }
.share-card p { margin: 0 0 25px; color: #866e65; line-height: 1.75; }
.share-card .share-help { margin-top: -9px; font-size: 13px; }
.share-help a { color: #9b584b; text-decoration: underline; text-underline-offset: 3px; }
.share-help a:hover { color: #763d33; }
.share-card>a, .share-actions a { display: inline-flex; min-height: 42px; align-items: center; justify-content: center; padding: 10px 18px; border-radius: 999px; color: #fff9f3; background: #ad7061; font-size: 13px; transition: background .2s ease, transform .2s ease; }
.share-card>a { margin-top: auto; }
.share-actions { display: flex; flex-wrap: wrap; gap: 10px; margin-top: auto; }
.share-card>a:hover, .share-actions a:hover { background: #925848; transform: translateY(-2px); }
.share-card a:focus-visible { outline: 3px solid #8c5143; outline-offset: 3px; }
.share-note { max-width: 1100px; margin: 25px auto 0; color: #9b8076; font-size: 12px; line-height: 1.7; }
@media(max-width:760px) {
  .share-page { min-height: calc(100vh - 64px); padding: 52px 20px 75px; }
  .share-intro { margin-bottom: 30px; }
  .share-intro-charm { display: none; }
  .share-grid { grid-template-columns: minmax(0, 1fr); gap: 16px; }
  .share-card { min-height: 280px; padding: 25px; }
  .share-card h2 { font-size: 30px; }
}
.admin-guestbook-icon { width: 24px; height: 24px; object-fit: contain; vertical-align: middle; }
@media (min-width: 761px) and (max-width: 1150px) {
  .topbar nav { display: none; position: absolute; top: 76px; left: 0; right: 0; padding: 20px; flex-direction: column; gap: 12px; background: #fff8ef; border-bottom: 1px solid #e6c9b9; }
