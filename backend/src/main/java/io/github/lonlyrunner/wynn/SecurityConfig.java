...
class SecurityConfig {
    ...
    @Bean SecurityFilterChain security(HttpSecurity http)throws Exception{return http.csrf(csrf->csrf.disable()).authorizeHttpRequests(auth->auth.requestMatchers("/api/public/**","/api/auth/**","/api/ai/**","/api/chat/**","/actuator/health").permitAll().requestMatchers("/api/admin/**").hasRole("OWNER").anyRequest().permitAll()).build();}
}
